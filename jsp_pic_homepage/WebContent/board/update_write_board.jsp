...
<body>
	<div id="wrapper">
      <div>
        <nav class="header_top">
				...
				<% if(session.getAttribute("sessionId") == null){ %>
					...
					<input type="button" value="로그인" onclick="open_win('BoardServlet?command=loginformAction')">
					<a href="BoardServlet?command=MemberFormAction">회원가입</a>
					</nav>
				<% } else { %>
					<nav class="member_login">
					<%= session.getAttribute("sessionId") %> 님
					<input type="button" value="로그아웃" onclick="window.location='/jsp_pic_homepage/board/logout.jsp'">
					</nav>
      ...
      <div class="content">
        ...
        <form name="frm" method="post" action="BoardServlet?command=update_write_board" enctype="multipart/form-data">
          <input type="hidden" name="command" value="update_write_board">
          <table border="1px solid black">
            ...
            <tr>
            	<th>파일첨부</th>
              <td>
                <input type="file" name="pic_pic"/>
              </td>
            </tr>
            <tr>
              <th>내용</th>
              <td>
                <textarea cols="80" rows="20" name="pic_con">${bVo.pic_con}</textarea>
              </td>
            </tr>
          </table>
        ...
        <div class="botton_button">
          <input id="btn_submit" type="submit" value="등록"/> <!-- onclick="return boardCheck() 구현 해야 함.  -->
          <input id="btn_submit" type="button" value="목록" onclick="location.href='BoardServlet?command=boardMain'"/>
        </div>
      </form>
